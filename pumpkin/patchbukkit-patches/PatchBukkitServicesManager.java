...
public class PatchBukkitServicesManager implements ServicesManager {
    ...
    public <T> void register(@NotNull Class<T> service, @NotNull T provider, @NotNull Plugin plugin, @NotNull ServicePriority priority) {
        RegisteredServiceProvider<T> rsp = new RegisteredServiceProvider<>(service, provider, priority, plugin);
        providers.computeIfAbsent(service, k -> new ArrayList<>()).add(rsp);
        providers.get(service).sort((a, b) -> b.getPriority().compareTo(a.getPriority()));
    }

    @Override
    public void unregisterAll(@NotNull Plugin plugin) {
        providers.values().forEach(list -> list.removeIf(rsp -> rsp.getPlugin().equals(plugin)));
    }
    ...
    public <T> void unregister(@NotNull Class<T> service, @NotNull Object provider) {
        ...
        if (list != null) list.removeIf(rsp -> rsp.getProvider().equals(provider));
    }
    ...
    public @NotNull List<RegisteredServiceProvider<?>> getRegistrations(@NotNull Plugin plugin) {
        ...
        providers.values().forEach(list -> list.stream().filter(r -> r.getPlugin().equals(plugin)).forEach(result::add));
        return result;
    }
    ...
    public <T> @NotNull Collection<RegisteredServiceProvider<T>> getRegistrations(@NotNull Class<T> service) {
        ...
        for (RegisteredServiceProvider<?> rsp : list) result.add((RegisteredServiceProvider<T>) rsp);
        return result;
    }
